...
def build():
    ...
    for t in TOPICS:
        ...
        personal=sum(p.get('kind','personal')=='personal' for p in d['photos'].values())
        supplement=len(d['photos'])-personal
        image_note=f'{personal}张参观照片'+(f' + {supplement}张补充图' if supplement else '')
        entries.append(f'<a class="issue-link" href="../{t["id"]}/"><img src="../{t["id"]}/images/{w["photo"]}" alt="{e(w["title"])} · Merton参观照片" width="{p["width"]}" height="{p["height"]}" loading="lazy"><div><p class="eyebrow">{t["dates"]}</p><h2>{t["title"]}</h2><p>{t["intro"]}</p><p class="issue-meta">{len(d["works"])}件作品（{t["note"]}） · {image_note}<br>{t["routes"]}</p><span class="text-link">进入这一辑 →</span></div></a>')
    base=ROOT/'museum-notes/european-painting';base.mkdir(exist_ok=True)
    (base/'index.html').write_text(f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>欧洲绘画 · 博物馆札记 | Merton</title><meta name="description" content="Merton看过的欧洲绘画：达·芬奇、卡拉瓦乔与维米尔。按画家整理参观照片，也比较题材、构图与收藏故事。"><link rel="stylesheet" href="../museum.css"></head><body><header class="site-header"><a class="signature" href="../">Merton</a><a href="../">博物馆札记 ↗</a></header><main class="wrap notebook-index"><p class="eyebrow">MUSEUM NOTES / EUROPEAN PAINTING</p><h1>欧洲绘画</h1><p class="index-intro">沿着一位画家，<br>把散在不同城市的相遇连起来。</p><section aria-labelledby="europe-artists"><p class="eyebrow">ARTIST NOTEBOOKS</p><h2 id="europe-artists">从画家开始</h2>{''.join(entries)}</section><aside class="notebook-about"><h2>以后慢慢展开</h2><p>这个板块会继续收录在欧洲和美国见到的绘画。可以沿着一位画家读，也可以比较同一个题材在不同年代的样子。</p></aside></main><footer class="site-footer wrap"><a href="../">← 博物馆札记</a><span>参观记录 / Merton · 图片来源逐件标注</span></footer></body></html>''')
