...
vermeer_data = json.loads((ROOT/'museum-notes/vermeer/data/works.json').read_text())
europe_supplements = sum(p.get('kind') == 'supplement' for p in vermeer_data['photos'].values())
europe_count = len(vermeer_data['works']) + caravaggio_count + len(leonardo_data['works'])
europe_photos = len(vermeer_data['photos']) - europe_supplements + caravaggio_photos + len(leonardo_data['photos'])
